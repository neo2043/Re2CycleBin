...
std::vector<std::string> print_attribute_data(std::shared_ptr<MFTRecord> record, PMFT_RECORD_ATTRIBUTE_HEADER pAttribute, ULONG32 cluster_size)
{
	...
	if (pAttribute->FormCode == NON_RESIDENT_FORM)
	{
		...
		auto temp_dataruns = record->read_dataruns(pAttribute);
		...
		if (temp_dataruns.has_value())
		{
			...
			for (const auto& run : temp_dataruns.value())
			{
				if (run.offset == 0)
				{
...
std::vector<std::string> print_attribute_index_allocation(PMFT_RECORD_ATTRIBUTE_HEADER pIndexAttrHeader, std::shared_ptr<MFTRecord> record, ULONG32 cluster_size, std::vector<std::shared_ptr<IndexEntry>> entries, bool full)
{
	std::vector<std::string> ret;
	...
	if (pIndexAttrHeader->FormCode == NON_RESIDENT_FORM)
	{
		...
		auto temp_dataruns = record->read_dataruns(pIndexAttrHeader);
		if (!temp_dataruns.has_value()) {
			win_error::print(temp_dataruns.error().add_to_error_stack("Caller: Datarun Error", ERROR_LOC));
		}
		if (temp_dataruns.has_value())
	...
	while (pAttribute->TypeCode != $END)
	{
		...
		case $INDEX_ROOT:
		{
			...
			if (!temp_index.has_value()) {
				win_error::print(temp_index.error().add_to_error_stack("Caller: Index Error", ERROR_LOC));
			}
			fr_attributes->add_item_multiline(print_attribute_index_root(pattr, temp_index.value()));
			break;
		}
		case $REPARSE_POINT:
		{
		...
		case $INDEX_ALLOCATION:
		{
			...
			fr_attributes->add_item_multiline(print_attribute_index_allocation(pAttribute, record, cluster_size, temp_index.value(), true));
			break;
		}
		case $DATA:
		{
...
int print_mft_info(std::shared_ptr<Disk> disk, std::shared_ptr<Volume> vol, std::string from, int64_t inode)
{
	...
	auto temp_record = commands::helpers::find_record(explorer, from, inode);
	...
	std::cout << std::boolalpha << MFTRecord::is_valid(record->get_record()->data()) << std::endl;

	utils::ui::title("MFT (inode:" + std::to_string(record->header()->MFTRecordIndex) + ") for " + disk->name() + " > Volume:" + std::to_string(vol->index()));
	
	print_mft_info_details(record, explorer->reader()->sizes.cluster_size);
...
int main () {
    ...
    if (volume != nullptr)
    {
        // print_mft_info(disk, volume, "D:\\all\\prog_elec_phil\\projects\\ReRecycleBin\\mytry\\rust\\one\\target\\debug\\build\\proc-macro2-18b24de3376e47e4", 0);
        // print_mft_info(disk, volume, "C:\\Users\\AUTO\\Desktop\\r", 0);
        print_mft_info(disk, volume, "", 305001);
    } else {
