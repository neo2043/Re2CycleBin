...
public:
	...
	// explicit Buffer(SIZE_T size, bool sector_aligned) 
	// : _mem(nullptr), _size(0), _sector_aligned(sector_aligned) {
	// 	if (sector_aligned) {
	// 		_mem = reinterpret_cast<T>(VirtualAlloc(
	// 			nullptr,
	// 			size,
	...
	// 		if (_mem) {
	// 			_size = size;
	// 			// _sector_aligned = sector_aligned;
	// 		}
	// 	} else {
	// 		_mem = reinterpret_cast<T>(malloc(size));
	// 		if (_mem) {
	// 			memset(_mem, 0, size);
	// 			_size = size;
	// 		}
    ...
    cppcoro::generator<std::pair<PBYTE, DWORD>> process_data(DWORD block_size)
    {
        ...
        {
            co_yield std::pair<PBYTE, DWORD>(reinterpret_cast<PBYTE>(_mem) + offset, static_cast<DWORD>((std::min)(_size - offset, block_size)));
        }
    }

    std::expected<bool, win_error> to_file(std::wstring filename) {
        DWORD dwWritten = 0;
        BOOL ret = FALSE;
    ...
    static std::expected<std::shared_ptr<Buffer<T>>,win_error> from_file(std::wstring filename) {
        DWORD dwWritten = 0;
        ...
        if (dwSizeOfFile != INVALID_FILE_SIZE) {
            ...
            if (!ReadFile(
                hFile,
                reinterpret_cast<LPVOID>(ret->data()),
                ret->size(),
                &dwWritten,
                NULL
            )) {
