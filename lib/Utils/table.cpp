	...
	for (i = 0; i < this->headers.size(); i++) {
		...
		for (unsigned int j = 0; j < this->data.size(); j++) {
			...
			for (unsigned int k = 0; k < this->data[j][i].size(); k++) {
				cell_max_width = max(cell_max_width, (uint32_t)utils::strings::utf8_string_size(this->data[j][i][k]));
			}
			m = max(m, cell_max_width);
		}
		for (unsigned int j = 0; j < this->headers[i].size(); j++)
		{
			m = max(m, (uint32_t)utils::strings::utf8_string_size(this->headers[i][j]));
		}
		column_size[i] = m;
		line_size += m;
	}
