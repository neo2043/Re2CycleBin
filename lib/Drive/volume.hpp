...
class Volume {
    ...
    private:
        ...
        std::expected<void, win_error> _detect_bitlocker(HANDLE h, PARTITION_INFORMATION_EX p);

        std::expected<void, win_error> _get_info_by_name(PWCHAR volume_name);

        std::expected<void, win_error> _detect_filesystem(PARTITION_INFORMATION_EX p);
